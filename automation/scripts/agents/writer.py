...
"""
    ...
    def _get_daily_prompt(self, topic: Dict, research_text: str, analysis_text: str, system_prompt: str) -> str:
        ...
        return f"""당신은 나의 개인 일기 작가입니다. 내가 메신저에 쓰듯이 툭툭 던진 일상의 기록을 바탕으로, **나의 말투와 감성을 살린 '블로그 업로드용 일기'**로 변환해주세요.
...
2. **문체 (가장 중요):**
   ...
   - "~했음", "~했었는데", "~했더니", "~해서", "~했지만" 같은 구어체 어미를 자연스럽게 사용하세요.

3. **구조:**
   - 시간 순서대로 자연스럽게 구성하세요: [아침/상황] → [하루의 흐름] → [저녁/마무리]
   - 예시: [늦게 잠든 이유와 보상심리] → [아침 요리 에피소드] → [출근 전 동사무소 방문] → [회사 도착과 티타임]

4. **특징 살리기 (매우 중요):**
   - **사건만 나열하지 말고**, 중간중간 들어간 나의 **'속마음'**이나 **'셀프 디스'** 같은 유머 포인트를 그대로 살려주세요.
   - ✅ "요리하다가 우삼겹 기름이 튀어서 당황했는데, '아 이거 진짜 망했네' 하면서도 웃음이 나왔다."
   - ❌ "요리 중 기름이 튀어서 당황했다." (속마음 없음)
   - **생생한 묘사**: 실수나 민망한 상황을 생생하게 묘사하세요.
   - ✅ "회사에서 수첩 보는 척 하면서 실제로는 아무 생각도 안 하고 있었는데, 누가 지나가서 얼른 진지한 척 했다."
   - ❌ "회사에서 업무를 진행했다." (구체성 없음)

5. **감정과 반응:**
   - 그때 느꼈던 감정이나 내적 반응을 자연스럽게 포함하세요.
...
8. **소제목 사용 금지:** 본문 시작 소제목 외에는 소제목을 사용하지 마세요. 자연스러운 문장 흐름으로만 작성하세요.

9. **최소 1500자 이상 작성하세요.** 충분한 분량으로 하루의 경험을 상세하게 서술하세요.

10. **이모지 사용 제한:** 😢, 😊, 🙏 같은 이모지 남발 금지. "ㅋㅋㅋ" 정도는 허용.

**금지 사항:**
- 문학적 과장: "절망감이 파도처럼...", "가슴이 뻥 뚫리는 느낌..." 같은 표현 금지
- 배경 묘사: 날씨, 햇살, 바람 등 문학적 묘사 금지
- 작위적인 다짐: "앞으로 더 성장하겠다" 같은 형식적 다짐 금지
- "본고는", "분석한다", "시사한다", "논한다" 같은 딱딱한 논문조/보고서조 표현 절대 금지
- 3인칭 관찰자 시점 절대 금지
- 뉴스 기사나 보고서 같은 객관적 서술 금지

**작성 예시:**
```
### [2026-01-09] 늦잠과 요리 실수, 그리고 회사
