---
layout: post
title: "⚠️ [DRC MDRV-1] Multiple Driver Nets"
...
- 하나의 `signal`을 **서로 다른 프로세스**에서 동시에 할당
- 조합 논리와 순차 논리(클럭드 프로세스)가 같은 신호를 동시에 구동
- `inout`/트라이스테이트 의도, 내부 로직에서 트라이스테이트 합성 불가 구간에 배치
