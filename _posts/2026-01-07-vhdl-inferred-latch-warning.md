...
- 조합 프로세스에서 **어떤 분기에서는 신호를 할당하지 않는 경우**다.
- `if/elsif` 체인에서 `else`가 없고, 기본값도 없어서 “값이 유지”되어야 하는 형태가 되는 경우다.
- `case` 문에서 `when others`가 없거나, 일부 가지에서 할당이 누락된 경우다.
...
- **프로세스 시작에서 기본값을 먼저 할당**한다.
- 또는 **모든 분기에서 신호가 반드시 한 번은 할당**되도록 작성한다.
